...
| L7a | Skip a follow-up, then continue | The closing screen opens with **Was uns noch fehlt**: the skipped field is listed with the reason, *Ergänzen* jumps back to its screen, and an "I don't know" shows the date they gave. With nothing open it reads "Nichts offen." Confirming is never blocked by it. |
...
| M3 | Toggle an add-on → **Änderungen senden** → submit | Heading *Anfrage aktualisieren*, no "More about your project" intake (that is for a first enquiry only), consent shown as already given, done variant "Danke, wir haben Ihre Änderungen erhalten." Same lead (no duplicate), `total_one_time` updated, `lead_versions` v1 + v2, team gets "Quote updated (v2)" with the diff. |
