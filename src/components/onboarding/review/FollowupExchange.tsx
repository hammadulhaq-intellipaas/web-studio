...
export function FollowupExchange({
  ...
  const answered = review?.history.length ?? 0;

  if (!question) {
    const doneText = textFor(definition.texts, answered ? 'review_intro' : 'review_nothing', locale);
    return (
      <section data-screen="onb-review-done" className="onb-reveal" style={{ paddingBottom: 72 }}>
        ...
        <div style={{ background: '#ffffff', border: `1px solid ${BORDER}`, borderRadius: 18, padding: '26px 28px', maxWidth: 680 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 15, fontWeight: 700, color: '#1E6E44', marginBottom: 10 }}>
            <span aria-hidden>✓</span> {answered ? t('done') : doneText?.title}
          </div>
          {!answered && doneText && (
            <div className="onb-markdown" style={{ fontSize: 14.5, color: BODY, lineHeight: 1.55 }}>
              <ReactMarkdown>{doneText.content_markdown}</ReactMarkdown>
            </div>
          )}
          <button
            type="button"
  ...
  return (
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 14 }}>
          ...
          {contextLabel && (
            <span style={{ fontSize: 12, fontWeight: 600, color: BLUE, background: '#EDF3FF', borderRadius: 999, padding: '3px 10px', maxWidth: '60%', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {contextLabel}
            </span>
          )}
        </div>
        <p data-testid="onb-followup-question" style={{ fontSize: 18, fontWeight: 700, lineHeight: 1.4, color: INK, margin: '0 0 18px' }}>
          {cap(question.question, locale)}
        </p>
        ...
        {question.quick_replies.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: choiceOnly ? 0 : 14 }}>
            {question.quick_replies.map((reply) => (
              ...
              >
                {replyLabel(question, reply.value, locale, targetField)}
              </button>
            ))}
          </div>
        )}
        ...
        .map((h) => {
          ...
          return (
            <li key={h.question_id} style={{ fontSize: 12.5, color: MUTED, background: '#F5F7FB', borderRadius: 10, padding: '8px 12px', display: 'flex', gap: 10 }}>
              <span style={{ flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cap(h.question, locale)}</span>
